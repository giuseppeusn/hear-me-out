...
$id_album = intval($_POST['album_id'] ?? 0);
...
if (!empty($mensagem) && $id_album > 0) {
    ...
    if ($stmt->execute()) {
        ...
        $stmt2 = $connection->prepare("INSERT INTO comentario_album (id_album, id_comentario) VALUES (?, ?)");
        $stmt2->bind_param("ii", $id_album, $id_comentario);

        if ($stmt2->execute()) {
            echo "Comentário registrado com sucesso";
        } else {
            echo "Erro ao relacionar comentário com álbum.";
        }
