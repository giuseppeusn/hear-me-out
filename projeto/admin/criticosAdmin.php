...
if ((isset($_SESSION['authenticated']) || $_SESSION['authenticated'] = true) && isset($_SESSION['admin'])) {
    if (isset($_GET['id']) && isset($_GET['aprovado'])) {
        ...
        if ($aprovado == 0 || $aprovado == 1) {
            $query_aprovar = "UPDATE critico SET aprovado = 1 WHERE id = $id";
...
<body>
    ...
    <div class="container mt-3">
        <table class="table table-striped">
            ...
            <tbody>
                <?php
                $query = "SELECT * FROM critico WHERE aprovado = 0"; // 0 = false
                ...
                foreach ($resultado_query as $i) {
                    ...
                    echo "<td>" . $i['email'] . "</td>";
                    echo "<td>" . $i['cpf'] . "</td>";
                    echo '<td><a href="maisInfo2.php?id=' . $i['id'] . '" class="btn btn-info btn-sm text-black dropdown-toggle ms-4">Detalhes </a></td>';
                    echo '<td>                        <a class="btn btn-success btn-sm me-1 aprovar-link" href="artistasAdmin.php?id=' . $i['id'] . '&aprovado=1">
